...
- **双因素认证 (TOTP)**：超级管理员与目录管理员均可开启基于时间的一次性密码两步验证，WEB 页面登录时密码与 6 位动态验证码在同一弹窗内一次提交，支持一键生成密钥、复制绑定链接。

- **仅验证码登录（免密，可选）**：在 TOTP 已开启的前提下，可在系统设置 / 目录编辑中勾选"仅验证码登录(免密)"，登录时跳过密码校验、仅输入 6 位动态验证码即可（属于单因素认证，安全性弱于"密码+验证码"，请谨慎使用）。
...
在程序目录下新建一个名为 `showpasswd` 的空文件，即可在主界面及目录编辑对话框中显示密码"眼睛"图标按钮，点击可临时查看已设置的密码；删除该文件则恢复默认的密码遮蔽。
