...
- **双因素认证 (TOTP)**：超级管理员与目录管理员均可开启基于时间的一次性密码两步验证，WEB 页面登录时需输入 6 位动态验证码，并支持一键生成密钥、复制绑定链接。

- **显隐密码（可选）**：程序目录下存在 `showpasswd` 文件时，GUI 密码输入框旁显示"显示"按钮，可临时查看明文（适用于单机、可信环境）；删除该文件即恢复默认遮蔽，无需重新初始化。
...
在程序目录下新建一个名为 `showpasswd` 的空文件，即可在主界面及目录编辑对话框中显示密码"显示"按钮，方便查看已设置的密码；删除该文件则恢复默认的密码遮蔽。
